...
public class TriangulationView: UIView {
    ...
    func generateVertices(_ size: CGSize,
                          cellSize: CGFloat,
                          variance: CGFloat = 0.75,
                          seed: UInt64 = UInt64.random(in: 0..<UInt64.max)) -> [Vertex] {
        ...
        for index in stride(from: minX, to: maxX, by: cellSize) {
            for verIndex in stride(from: minY, to: maxY, by: cellSize) {
                let random = CGFloat.random(-variance, variance)
                let generator = CGFloat(generator.nextUniform())
                let verX = index + CGFloat.random(cellSize / 2, cellSize * 2)/2 + generator + random
                let verY = verIndex + CGFloat.random(cellSize / 2, cellSize * 2)/2 + generator + random
