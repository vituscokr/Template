...
enum  Router :URLConvertible {
    ...
    var httpMethod : HTTPMethod {
        switch(self) {
        case .latestPhotos(_) : return .get
        }
    }
...
//        let publisher =  APIManager.shared.session.request(self, method: self.httpMethod, parameters: parameters, encoding: self.encoding, headers: self.headers)
...
//    }
    ...
    func request<T:Decodable>(parameters: Parameters? = nil ) -> AnyPublisher<T, RequestError> {
        return APIManager.shared.session.request(self, method: self.httpMethod, parameters: parameters, encoding: self.encoding, headers: self.headers)
            .validate()
            .publishDecodable(type:T.self)
            .value()
            .mapError {
